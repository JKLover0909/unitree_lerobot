...
## 8A. C++ MJPEG Server Đã Viết Sẵn Trong Repo Laptop

Đã tạo một tool C++ đúng hướng tài liệu Unitree/librealsense:

```text
/home/jkl0909/code/Son/unitree_lerobot/robot_camera_cpp/
```

File chính:

```text
robot_camera_cpp/g1_d435i_mjpeg_server.cpp
robot_camera_cpp/CMakeLists.txt
robot_camera_cpp/README.md
```

Tool này chạy trên robot/NX:

```text
D435i -> librealsense2 C++ -> OpenCV JPEG encode -> HTTP MJPEG server
```

Copy từ laptop sang robot:
...
Laptop xem trực tiếp nếu route OK:
...
```text
Capture error: Frame didn't arrive within 15000
D435i pipeline stopped.
```

thì đó là timeout RealSense: pipeline mở được camera nhưng không nhận frame trong thời gian chờ. Bản code mới đã đổi thành:

```text
Capture timeout/error: ... Restarting D435i pipeline...
```

và sẽ tự restart camera, HTTP server vẫn sống.

Nếu timeout liên tục ngay sau start:
...
## 8B. C++ LowState Recorder Đã Viết Sẵn Trong Repo Laptop

Đã tạo tool C++ để chạy trực tiếp trên robot/NX và ghi thông số khớp từ DDS:

```text
/home/jkl0909/code/Son/unitree_lerobot/robot_state_recorder_cpp/
```

File chính:

```text
robot_state_recorder_cpp/g1_lowstate_recorder.cpp
robot_state_recorder_cpp/CMakeLists.txt
robot_state_recorder_cpp/README.md
```
...
Nếu vẫn lỗi `Could not find unitree_sdk2Config.cmake`, tìm SDK trên robot:

```bash
sudo find /usr /usr/local /home/unitree /home/unitree/Code -name 'channel_subscriber.hpp' -o -name 'libunitree_sdk2*'
```

Nếu có source/install root, ví dụ:

```text
/home/unitree/Code/unitree_sdk2
```
...
Sau này đồng bộ camera và joint state bằng:
